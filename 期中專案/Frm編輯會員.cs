...
namespace 後台
{
    public partial class Frm編輯會員 : Form
    {
		...
		List<string> sex_pkIndex = new List<string>();
        ...
        public tMember memeber
        {
            get 
            {
                ...
                _memeber.Birthday = dateTimePicker1.Value; //memeber.Birthday;
				_memeber.Sex = this.cboSex.SelectedItem.ToString();   //ComboUtil.GetItem(this.cboSex, this.cboSex.SelectedIndex).Value; //memeber.Sex;
            ...
            set { 
				...
				dateTimePicker1.Value = Convert.ToDateTime(_memeber.Birthday);
				if (_memeber.Sex != null)
				{
					cboSex.SelectedIndex = sex_pkIndex.IndexOf(_memeber.Sex);
				}
				else
				{
				...
				_ImageStored = _memeber.MemberPhoto.ToString();
				if (!string.IsNullOrEmpty(_ImageStored))
				{
					byte[] bytes = (byte[])(_memeber.MemberPhoto);
		...
		public Frm編輯會員()
        {
			...
			try
			{
				for (int i = 0; i <= 1; i++)
				{
					...
					ComboBoxItem itemVip = new ComboBoxItem(i, text);
					cboVIP.Items.Add(itemVip);
					vip_pkIndex.Add(i);
				}
				...
				for (int i = 0; i <= 2; i++)
				{
					//string text = "否";
					string text = "";
					if (i == 0) { text = "未選擇"; } else if (i == 1) { text = "男"; } else { text = "女"; }
					//ComboBoxItem itemSex = new ComboBoxItem(i, text);
					//cboSex.Items.Add(itemSex);
					cboSex.Items.Add(text);
					sex_pkIndex.Add(text);
				}
